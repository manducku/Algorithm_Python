...
2 초	128 MB	1033	169	108	14.458%
문제
...
for list in command: #명령어 리스트 순회

    if(len(list) > 1):   #P 명령어가 들어왔을 때
            st_left.append(list[1])

    elif(list[0] == "L"):  #L 명령어가 들어왔을 때
        if(len(st_left) != 0):
            st_right.append(st_left.pop())

    elif(list[0] == "D"):  #D 명령어가 들어왔을 때
       if(len(st_right) != 0):
            st_left.append(st_right.pop())

    elif(list[0] == "B"):  #B 명령어가 들어왔을 때
        if(len(st_left) != 0):
            st_left.pop()

result = st_left + st_right[::-1]
print(''.join(result))
